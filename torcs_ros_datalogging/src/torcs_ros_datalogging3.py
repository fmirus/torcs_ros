...
class DataLogger:
    def __init__(self, logfile_name='log_1.h5', sensors_topic='torcs_ros/sensors_state',image_topic='torcs_ros/pov_image',control_topic='/torcs_ros/ctrl_state', scan_track_topic='torcs_ros/scan_track', 
                 global_pose_topic = '/torcs_ros/torcs_global_pose', global_speed_topic = '/torcs_ros/torcs_global_speed',
                 global_rpy_topic = '/torcs_ros/torcs_global_rpy',log_at_start=True):
        ...
        self.pub_global_pose =  rospy.Publisher('torcs_ros/synched/torcs_global_pose', PoseStamped, queue_size=50)
        self.pub_global_speed =  rospy.Publisher('torcs_ros/synched/torcs_global_speed', TwistStamped, queue_size=50)
        self.pub_global_rpy =  rospy.Publisher('torcs_ros/synched/torcs_global_speed', Vector3Stamped, queue_size=50)
        ##################################################
        
        #### Subscriptions #### #adding this many subs messed up timing on datalogger
        ...
        self.global_pose_sub = message_filters.Subscriber(global_pose_topic, PoseStamped)
        self.global_speed_sub = message_filters.Subscriber(global_speed_topic, TwistStamped)
        self.global_rpy_sub = message_filters.Subscriber(global_rpy_topic, Vector3Stamped)
        
        #######################
        
        #Function that synchronizes all input messages with same time stamp (within slop factor varicance)
#        self.synchronizer = message_filters.ApproximateTimeSynchronizer([self.image_sub, self.sensors_sub, self.control_sub, self.scan_sub, self.global_pose_sub,
#                                                                         self.global_speed_sub, self.global_rpy_sub], 20, 0.2)
        self.synchronizer = message_filters.ApproximateTimeSynchronizer([self.sensors_sub, self.control_sub, self.scan_sub, self.global_pose_sub, self.global_speed_sub, self.global_rpy_sub], 20, 0.2)
        #### callback functions ####
        self.synchronizer.registerCallback(self.cleanup_callback)
        ...
        self.synchronizer.registerCallback(self.global_callback)
...
#        self.data['ctrl_Accel2'] = [] #Gas Pedal (0 no gas, 1 full gas)
        ...
        self.data['ctrl_Stamp'] = []
        self.data['scan_track'] = []
        self.data['globalX'] = [] #Cars global X position
        self.data['globalY'] = [] #Cars global Y position
        self.data['yaw'] = []
        self.data['pitch'] = []
        self.data['roll'] = []
        ...
        for f in range(len(self.logfile_path)):#hardcoding for if cwd is within catkin workspace 
            try:
                if self.logfile_path[f:f+3] == "src":
                    self.logfile_path = self.logfile_path[0:f-1] + "/logged_data"
            except:
                pass
    ...
    def cleanup_callback(self, sensors_msg, control_msg, scan_msg, global_pose_msg, global_speed_msg, global_rpy_msg):
        for key in self.data:
            if key is 'lapTimes' or key is 'evalLapTimes':
                continue
            self.data[key] = []
    ...
    def sensors_callback(self, sensors_msg, control_msg, scan_msg, global_pose_msg, global_speed_msg, global_rpy_msg):
#    def sensors_callback(self, image_msg, sensors_msg, control_msg, scan_msg, global_pose_msg,
#                         global_speed_msg, global_rpy_msg):
    ...
    def control_callback(self, sensors_msg, control_msg, scan_msg, global_pose_msg, global_speed_msg, global_rpy_msg):    
#    def control_callback(self, image_msg, sensors_msg, control_msg, scan_msg, global_pose_msg,
#                         global_speed_msg, global_rpy_msg):
        if self.b_logging == True: #check whether logging is set to true
            ...
            self.data['ctrl_Stamp'].append([control_msg.header.stamp.secs, control_msg.header.stamp.nsecs])
            
#            self.data['ctrl_Stamp'][-1][0] = float(self.data['ctrl_Stamp'][-1][0])/(100)
#            self.data['ctrl_Stamp'][-1][1] = float(self.data['ctrl_Stamp'][-1][1])/(100)
            
#            self.write_ctrl_trigger()
            print("saved control")    
        
        
    def scan_track_callback(self, sensors_msg, control_msg, scan_msg, global_pose_msg, global_speed_msg,global_rpy_msg):
        if self.b_logging == True: #check whether logging is set to true
...
#            self.write_scan_trigger()
    ...
    def image_callback(self, sensors_msg, control_msg, scan_msg, global_pose_msg, global_speed_msg, global_rpy_msg):
...
#            print("saved img")
    ...
    def global_callback(self, sensors_msg, control_msg, scan_msg, global_pose_msg, global_speed_msg, global_rpy_msg):
        if self.b_logging == True:
            pass
            self.pub_global_pose.publish(global_pose_msg)
            self.pub_global_speed.publish(global_speed_msg)
            self.pub_global_rpy.publish(global_rpy_msg)
            self.data['globalX'].append(global_pose_msg.pose.position.x)
            self.data['globalY'].append(global_pose_msg.pose.position.y)

            self.data['roll'].append(global_rpy_msg.vector.x)
            self.data['pitch'].append(global_rpy_msg.vector.y)
            self.data['yaw'].append(global_rpy_msg.vector.z)
            self.write_global_trigger()
        
    # Print Message of time passed between saved samples
    def passed_time(self, sensors_msg, control_msg, scan_msg,global_pose_msg, global_speed_msg, global_rpy_msg):
        try:
            self.t2 = self.t1 
        except: 
            self.t2 = 0
...
#        hf['ctrl_Stamp'].resize((hf['ctrl_Stamp'].shape[0]+self.sample_buffer,2))
...
#        hf['ctrl_Stamp'][-self.sample_buffer:] = self.data['ctrl_Stamp']
...
#      self.data['scan_track'] = []       
    ...
    def write_global_trigger(self):
        hf = h5py.File(self.log_file, 'r+')        
        for strData in ['globalX', 'globalY', 'roll', 'pitch', 'yaw']:
            hf[strData].resize((hf[strData].shape[0]+self.sample_buffer,))
            hf[strData][-self.sample_buffer:] = self.data[strData][0]      
            del(self.data[strData])
            self.data[strData] = []  
        ...
        try:
          ...
          with h5py.File(self.log_file, 'w') as hf:
           #control data
            ...
            hf.create_dataset('ctrl_Stamp', (0,2), maxshape=(None,2),compression="gzip", compression_opts=9)
#            
#            #eval data
...
#            #sensory data
            ...
            hf.create_dataset('globalX', (0,), maxshape=(None,),compression="gzip", compression_opts=9)
            hf.create_dataset('globalY', (0,), maxshape=(None,),compression="gzip", compression_opts=9)
            hf.create_dataset('roll', (0,), maxshape=(None,),compression="gzip", compression_opts=9)
            hf.create_dataset('pitch', (0,), maxshape=(None,),compression="gzip", compression_opts=9)
            hf.create_dataset('yaw', (0,), maxshape=(None,),compression="gzip", compression_opts=9)
            hf.close()
